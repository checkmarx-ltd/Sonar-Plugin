...
public class ComputeSastMeasures implements MeasureComputer {
    ...
    public MeasureComputerDefinition define(MeasureComputerDefinitionContext def) {
        return def.newDefinitionBuilder()
                .setInputMetrics(SONAR_PROJECT_HAVE_SAST_RESULTS.key(),SAST_CRITICAL_VULNERABILITIES.key() ,SAST_HIGH_VULNERABILITIES.key(), SAST_MEDIUM_VULNERABILITIES.key(), SAST_LOW_VULNERABILITIES.key(),SAST_TOTAL_VULNERABILITIES.key(),
                		SAST_NEW_CRITICAL_VULNERABILITIES.key(),SAST_NEW_HIGH_VULNERABILITIES.key(), SAST_NEW_MEDIUM_VULNERABILITIES.key(), SAST_NEW_LOW_VULNERABILITIES.key(), SAST_TOTAL_NEW_VULNERABILITIES.key(), SAST_SCAN_DETAILS.key())
                .setOutputMetrics(SONAR_PROJECT_HAVE_SAST_RESULTS.key(),SAST_CRITICAL_VULNERABILITIES.key() , SAST_HIGH_VULNERABILITIES.key(), SAST_MEDIUM_VULNERABILITIES.key(), SAST_LOW_VULNERABILITIES.key(),SAST_TOTAL_VULNERABILITIES.key(),
                		SAST_NEW_CRITICAL_VULNERABILITIES.key(),SAST_NEW_HIGH_VULNERABILITIES.key(), SAST_NEW_MEDIUM_VULNERABILITIES.key(), SAST_NEW_LOW_VULNERABILITIES.key(), SAST_TOTAL_NEW_VULNERABILITIES.key(), SAST_SCAN_DETAILS.key())
                .build();
    }
    ...
    public void compute(MeasureComputerContext context) {
        if (context.getComponent().getType() != Component.Type.FILE) {
            if(isResult(context)){
                int sumCritical  = sumMetric(SAST_CRITICAL_VULNERABILITIES, context);
                ...
                int sumNewCritical  = sumMetric(SAST_NEW_CRITICAL_VULNERABILITIES, context);
                ...
                context.addMeasure(SAST_CRITICAL_VULNERABILITIES.key(), sumCritical);
                ...
                context.addMeasure(SAST_TOTAL_VULNERABILITIES.key(), sumCritical + sumHigh + sumMedium + sumLow);
                context.addMeasure(SAST_NEW_CRITICAL_VULNERABILITIES.key(), sumNewCritical);
                ...
                context.addMeasure(SAST_TOTAL_NEW_VULNERABILITIES.key(), sumNewCritical + sumNewHigh + sumNewMedium + sumNewLow);
            }
        }
    }
